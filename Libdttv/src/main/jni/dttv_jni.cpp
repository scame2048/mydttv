...
dttvListenner::dttvListenner(JNIEnv *env, jobject thiz, jobject weak_thiz)
{
    ...
    if (clazz == NULL)
    {
        LOGGER_V(TAG, "can not find DtPlayer \n ");
        return;
    }
    mClass  = (jclass) env->NewGlobalRef(clazz);
    mObject = env->NewGlobalRef(weak_thiz);
    LOGGER_V(TAG, "dttv listenner construct.");
}

dttvListenner::~dttvListenner()
{
    LOGGER_V(TAG, "dttv listenner destruct. enter");
    JNIEnv *env = ff_jni_get_env();
    env->DeleteGlobalRef(mObject);
    env->DeleteGlobalRef(mClass);
    LOGGER_V(TAG, "dttv listenner destruct.");
}
...
int dttvListenner::notify(int msg, int ext1, int ext2)
{
    ...
    {
        LOGGER_V(TAG, "updateState can not found ");
        return 0;
    }
    env->CallStaticVoidMethod(mClass, fields.post_event, mObject, msg, ext1, ext2, NULL);
    if(env->ExceptionCheck())
    {
        LOGGER_V(TAG, "An exception occurered while nofifying an event.\n ");
    }
    LOGGER_V(TAG, "NOtify with listener \n ");
    return 0;
}
...
static void jni_dttv_init(JNIEnv *env)
{
    ...
    // dai.android.learn.dttv
    clazz = env->FindClass("dai/android/learn/dttv/MediaPlayer");
    if (clazz == NULL)
    {
        return;
    }
...
static int jni_dttv_setup(JNIEnv *env, jobject obj, jobject weak_thiz)
{
    ...
    if (mp == NULL)
    {
        LOGGER_V(TAG, "Error: dtplayer create failed.");
        return -1;
    }
    ...
    LOGGER_V(TAG, "native dttv setup ok. lisenner addr:%p mp addr:%p", listenner, mp);
    return 0;
}
...
static int jni_dttv_release(JNIEnv *env, jobject thiz)
{
    ...
    if (mp)
    {
        ...
        LOGGER_V(TAG, "jni_dttv_release. lisenner addr:%p mp addr:%p", listenner, mp);
        if(listenner)
        {
            delete listenner;
        }
...
{
    LOGGER_V(TAG, "native_finalize");
    DTPlayer *mp = getMediaPlayer(env, thiz);
    if (mp != NULL)
    {
        LOGGER_V(TAG, "MediaPlayer finalized without being released");
    }
    ...
    jobjectArray keys, jobjectArray values)
{
    ...
    LOGGER_V(TAG, "setDataSource: path %s", tmp);
                                     ...
                                     jlong offset, jlong length)
{
    ...
    ALOGGER_V(TAG, "setDataSourceFD: fd %d", fd);
...
{
    LOGGER_V(TAG, "Enter prePareAsync");
    DTPlayer *mp = getMediaPlayer(env, thiz);
    if (mp == NULL)
    {
        return -1;
...
{
    LOGGER_V(TAG, "Enter seekTo pos:%d s", pos);
    DTPlayer *mp = getMediaPlayer(env, thiz);
    if (mp == NULL)
    {
        return -1;
...
int jni_dttv_stop(JNIEnv *env, jobject thiz)
{
    ...
    LOGGER_V(TAG, "native stop enter \n ");
    mp->stop();
    LOGGER_V(TAG, "native stop exit \n ");
    return 0;
}
...
jni_dttv_getTrackInfo(JNIEnv *env, jobject thiz, jobject sparse)
{
    ...
    if (info->has_video)
    {
        ...
        LOGGER_V(TAG, "video trackinfo: %s\n", buf);
    }
    ...
    if (info->has_audio)
    {
        ...
        LOGGER_V(TAG, "audio trackinfo: %s\n", buf);
    }
...
int jni_dttv_get_parameter(JNIEnv *env, jobject thiz, int cmd, jlong arg1, jlong arg2)
{
    ...
    if (mp == NULL)
    {
        LOGGER_V(TAG, "set parameter failed.mp == null.");
        return -1;
    }
    return 0;
}
...
static void jni_dttv_set_video_surface(JNIEnv *env, jobject thiz, jobject jsurface)
{
    ...
    //LOGGER_V(TAG,"NativeWindow Address:%p \n", window);
...
static int jni_dttv_set_audio_effect(JNIEnv *env, jobject thiz, jint id)
{
    ...
    if (mp == NULL)
    {
        LOGGER_V(TAG, "setDataSource, failed, mp == null ");
        return -1;
    }
...
static int register_natives(JNIEnv *env)
{
    ...
    if (clazz == NULL)
    {
        LOGGER_V(TAG, "Native registration unable to find MediaPlayer class.\n");
        return JNI_FALSE;
    }

    if (env->RegisterNatives(clazz, g_Methods, NELEM(g_Methods)) < 0)
    {
        LOGGER_V(TAG, "RegisterNatives failed.\n");
        return JNI_FALSE;
    }
...
jint JNI_OnLoad(JavaVM *vm, void *reserved)
{
    ...
    {
        LOGGER_V(TAG, "ERROR: GetEnv failed\n");
        goto bail;
    }
    assert(env != NULL);
    ...
    {
        LOGGER_V(TAG, "ERROR: MediaPlayer native registration failed\n");
        goto bail;
    }
