...
static int vo_surface_init(vo_context_t *voc) {
    ...
    vf_surface = {0};
    LOGGER_V(TAG,"android vo init OK, w:%d h:%d\n", context->dw, context->dh);
...
static int vo_surface_render(vo_context_t *voc, dt_av_frame_t *frame) {
    ...
    if (voc->para.device == NULL) {
        LOGGER_V(TAG,"android vo surface not set\n");
        return 0;
    }
    ...
    if (context->native_window_init == 0) {
        ...
        LOGGER_V(TAG,"android vo surface set ok. frame[%d-%d] window:[%d-%d]\n", frame->width, frame->height,
    ...
    if (vf->para.d_width != context->dw
        ...
        vf->para.d_pixfmt = DTAV_PIX_FMT_RGBA;
        LOGGER_V(TAG,"Need to Update Video Filter Parameter. w:%d->%d h:%d->%d pixfmt:%d->%d \n",
    ...
    if (ANativeWindow_lock(window, &buf, NULL) == 0) {
        ...
        LOGGER_V(TAG,"Render one frame. dstStride:%d.srcStride:%d dh:%d\n", dstStride, srcStride, context->dh);
        ...
        LOGGER_V(TAG,"Render one frame failed.\n");
        return 0;
    }
    LOGGER_V(TAG,"Render one frame ok.\n");
    return 0;
