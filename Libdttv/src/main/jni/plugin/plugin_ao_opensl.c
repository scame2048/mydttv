...
static int TimeGet(ao_context_t *aoc, int64_t *drift) {
    ...
    if ((res != SL_RESULT_SUCCESS)) {
        LOGGER_V(TAG,"Could not query buffer queue state in TimeGet (%lu)", (unsigned long) res);
        return -1;
    }
...
static int ao_opensl_write(ao_context_t *aoc, uint8_t *buf, int size) {
    ...
    if(!sys) {
        LOGGER_I(TAG, "Opensl empty. not render");
        return 0;
    }
    //LOGGER_I(TAG,TAG, "[%s:%d] start to write pcm: %d \n", __FUNCTION__, __LINE__, size);
#ifdef ENABLE_DTAP
    audio_effect_t *ae = (audio_effect_t *)wrapper->ao_priv;
    lock(&ae->lock);
    dtap_frame_t frame;
