    ...
    mDisplayWidth(0)
{
    ...
    mSurface      = -1;
    mRenderType   = 0;
    LOGGER_V(TAG,"dtplayer constructor ok \n");
}
...
DTPlayer::~DTPlayer()
{
    ...
    LOGGER_V(TAG,"dtplayer destructor called \n");
}
...
{
    LOGGER_V(TAG,"[%lu:%p] [%u:%p]", sizeof(mListenner), this->mListenner, sizeof(listenner), listenner);
...
{
    LOGGER_V(TAG,"Use GLSurfaceView. Register gl render \n");
    mRenderType = 0;
}
...
int DTPlayer::supportMediaCodec()
{
    ...
    if (ret < 0)
    {
        LOGGER_V(TAG,"Get mediainfo failed, quit \n");
        return 0;
    }
...
void DTPlayer::setupRender()
{
    ...
    if(info.has_sub)
    {
        ...
        LOGGER_I(TAG,"register android vo.\n");
    }
...
int DTPlayer::setDataSource(const char *file_name)
{
    ...
    if (mDtpHandle != NULL)
    {
        LOGGER_V(TAG,"last player is running\n");
        goto FAILED;
    }
    ...
    if (!handle)
    {
        LOGGER_V(TAG,"player init failed \n");
        goto FAILED;
    }
    //get media info
    ...
    mDtpHandle = handle;
    LOGGER_V(TAG,"Get Media Info Ok,filesize:%ld fulltime:%ld S \n", info.file_size, info.duration);
...
int DTPlayer::start()
{
    ...
    if (status != PLAYER_PREPARED)
    {
        LOGGER_V(TAG,"player is running \n");
        goto END;
    }
...
int DTPlayer::seekTo(int pos) // ms
{
    ...
    if (status == PLAYER_RUNNING || status == PLAYER_PAUSED || status == PLAYER_SEEKING)
    {
        ...
        if (mSeekPosition < 0)
        {
            LOGGER_V(TAG,"seekTo execute \n");
            mSeekPosition = pos;
            dtplayer_seekto(handle, pos);
        }
        else
        {
            LOGGER_V(TAG,"seekTo is ececuting \n");
        }
    }
    else
    {
        LOGGER_V(TAG,"seekTo is not ececuting in status:%d \n", status);
    }
...
int DTPlayer::notify(void *cookie, dtp_state_t *state)
{
    ...
    {
        LOGGER_V(TAG,"Check hw codec crated or not. vcodec type:%d. hw init:%d \n", state->vdec_type,
        ...
        state->last_status == PLAYER_STATUS_SEEK_EXIT)
    {
        ...
        LOGGER_V(TAG,"set status to running from seek complete \n");
        dtp->status = PLAYER_RUNNING;;
        dtp->Notify(MEDIA_INFO);
    }

    LOGGER_V(TAG,"UPDATECB CURSTATUS:%x status:%d \n", state->cur_status, dtp->status);
    LOGGER_V(TAG,"CUR TIME %lld S  FULL TIME:%ld  \n", state->cur_time, state->full_time);
END:
